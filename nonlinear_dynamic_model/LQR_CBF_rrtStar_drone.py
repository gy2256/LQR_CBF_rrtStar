...
from LQR_nonlinear_drone_planning import LQRPlanner
...
class LQRrrtStar:
    ...
    def LQR_steer(self, node_start, node_goal, exact_steering=False):
        ...
        wx, wy, _, _, = self.lqr_planner.lqr_planning(node_start.x, node_start.y, node_goal.x, node_goal.y,
                                                      self.LQR_Gain, show_animation=show_animation,
    ...
    def cal_LQR_new_cost(self, node_start, node_goal, cbf_check=True):
        wx, wy, _, can_reach = self.lqr_planner.lqr_planning(node_start.x, node_start.y, node_goal.x, node_goal.y,
                                                             self.LQR_Gain, show_animation=False, cbf_check=cbf_check,
                                                             solve_QP=self.solve_QP)
        px, py, traj_cost = self.sample_path(wx, wy)
        if wx is None:
    ...
    def LQR_choose_parent(self, node_new, neighbor_index):
        ...
        for i in neighbor_index:
            ...
            _, _, _, can_reach = self.lqr_planner.lqr_planning(self.vertex[i].x, self.vertex[i].y, node_new.x,
                                                               node_new.y, self.LQR_Gain, show_animation=False,
                                                               solve_QP=self.solve_QP)

            if can_reach and not self.utils.is_collision(self.vertex[i],
                                                         node_new):  # collision check should be updated if using CBF
        ...
        node_new.parent.childrenNodeInds.add(
            len(self.vertex) - 1)  # Add the index of node_new to the children of its parent. This step is essential when rewiring the tree to project the changes of the cost of the rewired node to its antecessors
    ...
    def updateCosts(self, node):
        for ich in node.childrenNodeInds:
            self.vertex[ich].cost = self.cal_LQR_new_cost(node, self.vertex[ich], cbf_check=False)[
                0]  # FIXME since we already know that this path is safe, we only need to compute the cost
        ...
        if len(Vg_leaves) >= (self.adapIter * 30):  # The acceptable number of trajectories to adapat upon
            ...
            for vg in elite_Vg_leaves:
                ...
                while tStep < len(traj2vg[:, 1]):
                    ...
                    elite_cddtSample = [pi_q_tStep,
                                        vgcost2come]  # This tuple contains the actual sample pi_q_tStep and the CostToCome to the goal of the corresponding trajectory
    ...
    def CE_KDE_Sampling(self, frakX):
        ...
        if self.kde_enabled:  # self.params.kde_enabled:
            ...
            if self.plot_pdf_kde:
                ...
                CS = plt.contour(Xxgrid, Xygrid,
                                 grid_probs.reshape(Xxgrid.shape))  # , norm=LogNorm(vmin=4.18, vmax=267.1))
    ...
    def KLdiv(self, grid_probs):
        ...
        return -sum(
            [pre_grid_probs[i] * np.log2(grid_probs[i] / pre_grid_probs[i]) for i in range(len(pre_grid_probs))])
