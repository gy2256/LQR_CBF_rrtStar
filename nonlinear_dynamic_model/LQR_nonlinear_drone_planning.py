...
class LQRPlanner:
    ...
    def lqr_planning(self, sx, sy, gx, gy, LQR_gain, test_LQR=False, show_animation=True, cbf_check=True,
        ...
        while time <= self.MAX_TIME:
            ...
            if solve_QP:
                try:
                    u = self.cbf_rrt_simulation.QP_controller([x[0, 0] + gx, x[1, 0] + gy, x[2, 0] + gtheta], u,
                                                              model="unicycle")
                except:
        ...
        while time <= self.MAX_TIME:
            ...
            if cbf_check and not test_LQR:
                if not self.cbf_rrt_simulation.QP_constraint(
                        [x[0, 0] + gx, x[1, 0] + gy, x[2, 0] + gv, x[3, 0] + gtheta], u,
...
def main():
    ...
    for i in range(ntest):
        ...
        if not acceleration_model:
            rx, ry, error, foundpath = lqr_planner.lqr_planning(sx, sy, gx, gy, test_LQR=True,
                                                                show_animation=SHOW_ANIMATION)

        else:
            sv, stheta = 0, 0
            gv, gtheta = 0.1, np.pi / 4
            rx, ry, error, foundpath = lqr_planner.lqr_planning(sx, sy, sv, stheta, gx, gy, gv, gtheta, test_LQR=True,
                                                                show_animation=SHOW_ANIMATION)
